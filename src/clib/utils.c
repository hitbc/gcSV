...
void fexist_check(const char *filename){
	FILE* f = xopen(filename,"r");
	fclose(f);
}
...
uint64_t inline char2Kmer(const char *str_kmer, uint8_t L_kmer, uint8_t *Bit){
...
uint64_t inline char2KmerRC(const char *str_kmer, uint8_t L_kmer, uint8_t *Bit){
	xassert(L_kmer <= 32 ,"Length of kmer should be no more than 32 base pair.");
	uint64_t value = 0;
	for (uint8_t i=0; i<L_kmer;++i)
