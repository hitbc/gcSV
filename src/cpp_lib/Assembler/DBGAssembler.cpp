...
void KmerCounter::add_new_kmer(int mode, int p, int i, bool IsStoreReadInfo){
	...
	else if(mode == 1){//NGS read
		read_Ngs_n++;
		if(IsStoreReadInfo){
			NGS_rp.emplace_back(i, p);
		}
	}
	else if(mode == 2){//NGS read
		read_Tgs_n++;
		if(IsStoreReadInfo){
			TGS_rp.emplace_back(i, p);
		}
	}
}

void DBG_INFO_item::showData(){
	if(depth_read <= 1 && depth_ref == 0){ fprintf(stderr, "SKIP-D1"); return;	}
	fprintf(stderr, "\n");
...
void UNITIG_INFO_item::setWithRead(int headNodeId, int endNodeId, std::vector<DBG_INFO_item *>& id2info, int MIN_DEPTH){
	...
	if(withLowDepthKmerRead){
		if(false)fprintf(stderr, "unitig (%d) is set unused because with low depth kmer\n", id);
	}
...
void UNITIG_INFO_item::setBranchUni(std::vector<DBG_INFO_item *>& id2info){
	...
	if(inBranchUni.empty()) inBranchUni.emplace_back(-1);
	for(int i: outBranchKmer)
		outBranchUni.emplace_back(id2info[i]->unitigId);
	if(outBranchUni.empty()) outBranchUni.emplace_back(-1);
...
void UNITIG_INFO_item::show(std::vector<DBG_INFO_item *>& id2info){
	...
	//the read position
	for(ReadPos & p : getAllReadTGS(id2info)){
		p.show(stderr);
	}

	fprintf(stderr, "[TGS read position END]");
	//the read position
	for(ReadPos & p : getAllReadTGSTail(id2info)){
		p.show(stderr);
	}
...
//	for(int i:kmer_list)
//		fprintf(stderr, "%d", id2info[i]->depth_ref);
//	fprintf(stderr, "\n");
	//id2info[i]->showDataSimple();
	//for(int i:kmer_list) id2info[i]->showDataDetail();
}

void MainDBGHandler1::addRef(uint8_t *tseq, int tlen, Contig_String_aligner *ca){
	std::string ref;
...
bool Path::isSAMEwith(Path &c){
	return (bgnUID == c.bgnUID
			&& endUID == c.endUID
			&& SV_len == c.SV_len
			&& unitigPL.size() == c.unitigPL.size()
			&& type == c.type
			&& ID_SUM == c.ID_SUM
			&& pathKey == c.pathKey
	);
...
bool Path::isSAMENodeSet(Path &c){
	return (bgnUID == c.bgnUID
			&& endUID == c.endUID
			&& UNIQ_ID_product == c.UNIQ_ID_product
			&& UNIQ_ID_SUM == c.UNIQ_ID_SUM
	);
}
...
void Path::showSim(std::vector<DBG_INFO_item *> &id2info){
	...
	if(true)fprintf(stderr, "Show all support read for this path");
...
void Path::show(std::vector<DBG_INFO_item *> &id2info){
	...
	int global_pos = 0; int offset = 0;
	if(true){
		for(UNITIG_INFO_item* up: unitigPL){
			int pos_ref = up->get_ref_pos(id2info);
			if(pos_ref == -2) offset = -999;//repeat ref
			else if(pos_ref == -1) offset = -1999;// not include ref
			else offset = pos_ref - global_pos;
			fprintf(stderr, "%d--", up->id);
			if(false)fprintf(stderr, "[%2d %3ld (refPOS %5d : %5d %5d) (repeat %d %3d)]--->\n",
...
void PathCluster::updateStepCountByNewSignal(){
	...
	if(greatestCommonDivisor ==0 ) return;
...
unsigned searchRepeatsUnitig(
	...
	for(int i:cu.outBranchUni){
		if (i == -1){/*Do nothing*/} //C1: end node
		else if(i == cu.id){ //C2: point to it-self
			cu.isRepeat = true;
			cu.finalRootId = cu.id;
		}
		else if (unitig_l[i].loopCurr == 0) { // the successor word has not been visited
			...
			cu.loopRoot = MIN(cu.loopRoot, unitig_l[i].loopRoot);
		else{/*Do nothing*/}//reach other node, but this node is not in the same loop with it
	}
...
bool MainDBGHandler1::buildDBG(int minDepthNGS, int minDepthTGS) {
	...
	for (KmerCounterTable::value_type &kmerNumItem : kmerCountingTable){
		if(kmerNumItem.second.is_low_depth(minDepthNGS, minDepthTGS)){
			kmer_LOW += kmerNumItem.second.read_Ngs_n + kmerNumItem.second.read_Tgs_n;
			continue;
		}
		int read_count = 0;
		if(kmerNumItem.second.read_Ngs_n >= minDepthNGS)
			read_count = kmerNumItem.second.read_Ngs_n;
	...
	for( std::unordered_map<std::string, DBG_INFO_item>::value_type &kmerIdx_i : kmerIdx){
		...
		for (const char symbol : alphabet) {
			...
			if (kmerIdx.count(nextKmer) == 0) 		   		 continue;   // the successor word does not exist in the reads
	...
	//set begin and end
	std::vector<int> headKmers; std::vector<int> tailKmers;
	...
	if(REPEAT_DEBUG) {for( DBG_INFO_item * kmerIdx_i : id2info) kmerIdx_i->showData();}

	//debug show all untigs
	for( UNITIG_INFO_item & cu: unitig_l){
		if(REPEAT_DEBUG) cu.show(id2info);
	}

	if(REPEAT_DEBUG)printf("END");
	return withLoop;
}
...
void MainDBGHandler1::showDBG_graph(){
	if(false){
		...
		for(KmerCounterTable::iterator it = kmerCountingTable.begin(); it != kmerCountingTable.end(); it++){
			if(it->second.read_Ngs_n > 1|| it->second.refN > 0)
				fprintf(stderr, "%s: [ %d | %d ]\n", it->first.c_str(), it->second.read_Ngs_n, it->second.refN);
		}
	}
	fprintf(stderr, "\n\nShow All Unitigs\n\n");
	if(true) for( UNITIG_INFO_item & cu: unitig_l){ cu.show(id2info); }
...
void MainDBGHandler1::kmerCountingTest(const unsigned kmerSize) {
	...
	for (unsigned strIndex = 0; strIndex < strCount; ++strIndex){
		...
		for (unsigned j = 0; j < kmer_number; ++j) {
			...
			if(it == kmerCountingTable.end() || it->second.read_Ngs_n < 3){
			}else
				usedKmer++;
		}
...
void MainDBGHandler1::errorCorrection(const unsigned kmerSize, int minRight, int maxError) {
	...
	for (unsigned strIndex = 0; strIndex < strCount; ++strIndex){
		...
		unsigned kmer_number = seqLen - kmerSize + 1;
		bool withFail = false;
		if(false)fprintf(stderr, "Before correction %s \n", seq.c_str());
		for (unsigned j = 0; j < kmer_number; ++j) {
			std::string word(seq.substr(j, kmerSize));
			KmerCounterTable::iterator it = kmerCountingTable.find(word);
			if(it == kmerCountingTable.end() || it->second.read_Ngs_n <= maxError){//need correction
				char lastBase = word.back();
				int bestBase = -1; int bastCount = 0;
				for(int i = 0; i < 4; i++){
					if(lastBase == "ACCT"[i])
						continue;
					word[word.size() - 1] = "ACGT"[i];
					it = kmerCountingTable.find(word);
					if(it != kmerCountingTable.end() && ((it->second.read_Ngs_n >= minRight && it->second.read_Ngs_n > bastCount) )) {//|| (bestBase != -1 && it->second.refN != 0)
						bestBase = i; bastCount = it->second.read_Ngs_n;
					}
				}
				if(bestBase != -1){
					if(lastBase != 'N') correctBaseACGT++;
					else correctBaseN ++;

					reads_str[strIndex][j+ kmerSize - 1] = "ACGT"[bestBase];
					if(false) fprintf(stderr, "Base correct readID %5d pos %3d\n", strIndex, j + kmerSize - 1);
				}else{
					withFail = true;
					if(false) fprintf(stderr, "Base correct FAILED: pos %3d\n", j + kmerSize - 1);
				}
			}
		}
		if(withFail){
			...
			for (unsigned j = 0; j < kmer_number; ++j) {
				...
				if(it == kmerCountingTable.end() || it->second.read_Ngs_n <= maxError){//need correction
					...
					for(int i = 0; i < 4; i++){
						...
						word[0] = "ACGT"[i];
						it = kmerCountingTable.find(word);
						if(it != kmerCountingTable.end() && ((it->second.read_Ngs_n >= minRight && it->second.read_Ngs_n > bastCount)) ){//
							bestBase = i; bastCount = it->second.read_Ngs_n;
						}
					}
					if(bestBase != -1){
						if(firstBase != 'N') correctBaseACGT++;
						else correctBaseN ++;
						reads_str[strIndex][seq.size() - j - kmerSize] = "ACGT"[bestBase];
						if(false) fprintf(stderr, "Base correct (Reverse) readID %5d pos %3ld\n", strIndex, seq.size() - j - kmerSize);
					}else{
						withFail = true;
						if(false) fprintf(stderr, "Base correct (Reverse) FAILED: pos %3ld\n", seq.size() - j - kmerSize);
					}
				}
			}
		}
		...
		if(correctBaseACGT > 1 || correctBaseN > 1){
			...
			if(false)fprintf(stderr, "Base correct readID %5d \t", strIndex);
			if(false)fprintf(stderr, "correctBaseN %d correctBaseN %d\n", correctBaseACGT, correctBaseN);
		}
		if(false) fprintf(stderr, "correctBaseN %d \n", correctBaseACGT);
		if(false) fprintf(stderr, "strIndex %d: After  correction %s \n\n", strIndex, seq.c_str());
	}
}
