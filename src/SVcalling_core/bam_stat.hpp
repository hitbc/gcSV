...
struct BAM_STATUS
{
									...
									std::vector<float> &UM_stPos_distribution, int &START_OFFSET_UM)
	{
		...
		int min_probability_size = minInsertLen - 2 * analysis_read_length;
		if (min_probability_size < 1)
			min_probability_size = 1;
		...
		for (int i = min_probability_size; i < max_probability_size; i++)
		{
			...
			for (int j = 0; j < i; j++)
				DR_bp_distribution[j] += PI_per_position;
		}
		float sum_p = 0;
		for (unsigned int i = 0; i < DR_bp_distribution.size(); i++)
			sum_p += DR_bp_distribution[i];
		float levelup_rate = 1 / sum_p;
		for (unsigned int i = 0; i < DR_bp_distribution.size(); i++)
			DR_bp_distribution[i] *= levelup_rate; // sum will be 1
		...
		for (int i = 0; i < max_probability_size_SH; i++)
			SH_bp_distribution[i] /= SH_bp_distribution_sum;
