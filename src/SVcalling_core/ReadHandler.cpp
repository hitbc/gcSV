...
//core function to store NGS reads
...
void BAM_handler::store_LRS_ReadCore(int readLen, uint8_t *seq, uint8_t * qual, int cigarLen, uint32_t* bam_cigar,
		const char * bam_qname, int primary_chrID, int primary_pos,
		int32_t pos, int direction){//check
	...
	rl->back().set(bam_qname, primary_chrID, primary_pos, 0, direction, pos, readLen, cigarLen, base.n, quality.n, cigar.n, 0, 0, 0);//new a read record
	//show data:
	if(false){
		//seq
...
void BAM_handler::storeReadCore(//check
	...
	if(t == Read_type::SR){
		...
		rl->emplace_back();
		rl->back().set(NULL, -1, -1, flag, direction, pos, readLen, cigarLen, base.n, quality.n, cigar.n, soft_left, soft_right, gap_mismatch_inside);//new a read record
		rl->back().read_end = -1;
	}
	else{
		READ_LIST *rl =&um_read_list;
		rl->emplace_back();
		rl->back().set(NULL, -1, -1, flag, 1 - mate_direction, pos, readLen, -1, base.n, quality.n, cigar.n, -1, -1, -1);//new a read record
		rl->back().read_end = -1;
	}
