...
void SveHandler::combine_repeat_tail_of_contigs(bool print_log, int ori_read_number,
...
//		}
		...
		getSuggestSVlength(contig_L);
		alignment_and_get_var(print_log, 0, contig_ID, contig_L.SUGGEST_pos_list[0].suggest_pos - target_ref_pos, contig_L.seq.size(), target_ref_len);
		//erase contig R
...
void SveHandler::get_BP_from_tran_based_ins(bool print_log, std::vector<Tran_ins_suggest_pos> &suggest_pos, std::vector<AssemblyContig> &contigs, std::vector<TRANS_INS_info> &bp_l){
	int target_tid; int target_ref_pos; int target_ref_len;
	ca.get_ref_info(target_tid, target_ref_pos, target_ref_len);
	uint8_t * target_ref = ca.get_ref();
	if(false){
...
//			if(sug_L->contig_type != 1)	std::swap(sug_L, sug_R);
	...
	if(sug_L_idx == -1 || sug_R_idx == -1){//no paired
		...
		if(max_repeat_idx != -1){
			...
			source_reference = ref->load_ref_by_region(ref_chr_ID, begin_load_position, end_load_position, &source_load_len);
			...
			for(uint i = 0; i < suggest_pos.size(); i++){
				if(suggest_pos[i].contig_ID != max_sug.contig_ID && suggest_pos[i].contig_type != max_sug.contig_type){
					...
					ca_re_locate.align_non_splice(&(bin_contig[0]), mate_contig.seq.size(), 0, source_load_len, 400);
	...
	if(sug_L_idx != -1 && sug_R_idx != -1){//successfully paired
		...
		for(int mode = 0; mode < 2; mode++){
			...
			if(sug_c->contig_is_revered_to_ref){
				...
				source_reference = ref->load_ref_by_region(source_tid, source_load_bg, source_load_bg + contig_len, &source_load_len);
...
void SveHandler::handle_tran_based_ins(bool print_log, int ori_read_number, std::vector<ASS_reads_info> &ass_read_list,
		std::vector<std::string> &read_list, std::vector<AssemblyContig> &contigs)
{
	...
	for (AssemblyContig &contig : contigs) {
		if(contig.ending_reason[0] != 0 ||  contig.ending_reason[1] != 0) continue;
		if(contig.contig_is_filtered) continue;
		with_part_ass_contig |= contig_signal_of_tran_based_ins(print_log, contig_ID, contig, ori_read_number, ass_read_list,read_list, ref, suggest_pos, sig_para.MaxReadLen);
		contig_ID++;
	}
	...
	get_BP_from_tran_based_ins(print_log, suggest_pos, contigs, bp_l);
	...
	if(with_part_ass_contig){
		...
		if(bp_l.size() == 2 &&  bp_l[0].is_R_not_L == false && bp_l[1].is_R_not_L == true){
			...
			char * source_reference = ref->load_ref_by_region(source_tid, source_load_bg, source_load_ed, &source_load_len);
...
//			//get variants
			...
			alignment_and_get_var(print_log, 0, -1,
