...
struct READ_record{
	...
	void set(const char * r_name, int primary_chrID, int primary_pos, uint16_t flag_, uint8_t direction_, uint64_t position_, int read_l_, int cigar_l_,
			int read_index_, int quality_index_, int cigar_index_,
			int soft_left_, int soft_right_, int NM_NUM_){
		this->r_name.clear();
		if(r_name != NULL)
...
struct ASS_reads_info{
	...
	void set(int read_in_ref_offset_, int read_st_pos_, int read_ed_pos_, int read_list_index_, int soft_left_, int soft_right_,
			int number_NM_, Read_type::T signal_type_, bool LRS_full_begin, bool LRS_full_end){
...
#define MIN_BASE_QUAL 6
	void add_read_bin(Read_type::T t, uint8_t * read_str, uint8_t * qual_str, int read_len, int read_in_ref_offset_, int left_clip, int right_clip, int NM_num, int read_list_index_){
		...
		ass_read_list.back().set(read_in_ref_offset_, read_st_pos, read_ed_pos, read_list_index_, left_clip, right_clip, NM_num, t, true, true);
...
//	        last_N_base = i;
	...
	void add_read_TL(char * read_str, uint8_t * qual_str, int read_len, bool store_in_reverse, int read_list_index_, int tid, int pos){
		...
		ass_read_list.emplace_back();
		ass_read_list.back().set(-1, tid, pos, read_list_index_, -1, -1, store_in_reverse, Read_type::TL, true, true);
		reads.emplace_back(read_str);
	}

	//adding LRS reads into assembly read list
	void add_read_LRS(std::string & read_str, int read_list_index_, int tid, int pos,
			bool is_full_begin, bool is_full_end, int soft_left, int soft_right){
		ass_read_list.emplace_back();
		ass_read_list.back().set(-1, tid, pos, read_list_index_, soft_left, soft_right, 0, Read_type::LRS, is_full_begin, is_full_end);
		reads.emplace_back(read_str);
	}
...
public:
	...
	void store_LRS_ReadCore(int readLen, uint8_t *seq, uint8_t * qual, int cigarLen, uint32_t* bam_cigar, const char * bam_qname, int primary_chrID, int primary_pos, int32_t pos, int direction);
