...
#include "SVcalling_core/sv_main.hpp"
...
int main(int argc, char *argv[])
{
	//fprintf(stderr, "\n\n main version V2.01\n\n");
	COMMAND_HANDLER ch;
	ch.add_function("call", "SV calling using (NGS or CCS or NGS+CCS)BAM file", SVLoci_main);
	ch.add_function("ngs_bam_stat", "Get NGS-BAM ISIZE distribution.", get_bam_statistics);
	ch.add_help_msg_back("Usage: ngs_bam_stat reference.fa NGS.bam/cram > STAT.json");
	ch.add_function("ngs_fa_stat", "Indexing the reference for NGS SV calling.", get_fa_statistics);
	ch.add_help_msg_back("Usage: ngs_fa_stat reference.fa > ref.idx");
	ch.add_function("ngs_trans_reads", "Extract trans_read from NGS BAM", transLocation_read_main);
	ch.add_help_msg_back("Usage: ngs_trans_reads reference.fa input.bam output.bam");
